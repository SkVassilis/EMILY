...
historia.append(train_model(model = model              # Model or already saved model from directory
                            ,dataset = data_fusion([CBC[0],NOISE[0]])              
                            ,epoch = 30               # Epochs of training
...
historia.append(train_model(model,data_fusion([CBC[1],NOISE[1]]), 40, 500, 0.1, save_model=False))
historia.append(train_model(model,data_fusion([CBC[2],NOISE[2]]), 40, 500, 0.1, save_model=False)) 
historia.append(train_model(model,data_fusion([CBC[3],NOISE[3]]), 40, 500, 0.1, save_model=False))
historia.append(train_model(model,data_fusion([CBC[4],NOISE[4]]), 40, 500, 0.1, save_model=False))
historia.append(train_model(model,data_fusion([CBC[5],NOISE[5]]), 40, 500, 0.1, save_model=False))
historia.append(train_model(model,data_fusion([CBC[6],NOISE[6]]), 80, 500, 0.1, save_model=ntype+'hlv_30TO10')) 



save_history(historia,name=ntype+'hlv_30TO10',save=True,extendend=True)
